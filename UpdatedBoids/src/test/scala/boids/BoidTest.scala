...
class BoidTest extends munit.FunSuite:
  ...
  //Can't use runTestCase for Migratory bird as they have a different behavior hard to predict

  test("Boid initialized with 2 vectors gives regular boid"):
    val bird = Boid(Vector2(0, 0), Vector2(0, 0))
    assert(!bird.isInstanceOf[Migratory])

  test("Boid initialized with direction vectors (1,1) gives migratory bird"):
    val bird = Boid(Vector2(0, 0), Vector2(0, 0),direction =   Vector2(1, 1))
    assert(bird.isInstanceOf[Migratory])

  test("Boid initialized with direction (0,0) gives regular boid"):
    val bird = Boid(Vector2(0, 0), Vector2(0, 0),direction =   Vector2(0, 0))
    assert(!bird.isInstanceOf[Migratory])
