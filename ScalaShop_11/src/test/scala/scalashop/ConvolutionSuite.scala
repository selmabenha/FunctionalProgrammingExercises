...
class ConvolutionSuite extends scalashop.ImageTestSuite:
  ...
  test("convolution: convolution should not flip asymmetric kernel (10 pts)") {
    val kernel = Kernel(
      3,
      3,
      Array(
    ...
    assertEquals(Convolution(source, kernel).toImage, target)
  }
  ...
  (1 to 10).foreach(_ =>
    test("convolution: random 3x3 image under the identity kernel should be unchanged (5 pts)") {
      ...
      assertEquals(Convolution(source, kernel).toImage, source)
    }
