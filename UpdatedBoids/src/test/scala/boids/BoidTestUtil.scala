...
def runTestCase(name: String)(using munit.Location) =
  ...
  testCase("reference")
    ...
    .foldLeft(initBoids) { (world, expected) =>
      val next = tickWorld(world, physics, 0)
...
def seqBoidToJson(boids: Seq[Boid]) =
  boids.map(b =>
    if b.isInstanceOf[Migratory] then
      ujson.Obj(
        "x" -> b.position.x,
        "y" -> b.position.y,
        "vx" -> b.velocity.x,
        "vy" -> b.velocity.y,
        "dx" -> b.asInstanceOf[Migratory].getDirection.x,
        "dy" -> b.asInstanceOf[Migratory].getDirection.y
      )
    else
      ujson.Obj(
        "x" -> b.position.x,
...
def generateTestCase(name: String, initialBoids: Seq[Boid], physics: Physics, steps: Int = 50) =
  ...
  val reference = Seq.iterate(seqBoidToBoidSequence(initialBoids), steps)(boids => tickWorld(boids, physics, 0))
...
def generateTestCases() =
  ...
  //Additional tests created for the unguided callback
  generateTestCase(
    "61_singleBirdNoForces",
    Seq(Boid(Vector2(1, -45), Vector2(2, 3),direction = World.CardinalDirection.North)),
    noForces
  )

  generateTestCase(
    "62_birdAndBirdNoForces",
    Seq(Boid(Vector2(1, -45), Vector2(2, 3),direction = World.CardinalDirection.North),
    Boid(Vector2(10, -45), Vector2(2, 3))),
    noForces
  )

  generateTestCase(
    "63_4birdsWithDiffMigrationForces",
    Seq(Boid(Vector2(1, -45), Vector2(2, 3),direction = World.CardinalDirection.North),
    Boid(Vector2(1, -45), Vector2(2, 3),direction = World.CardinalDirection.South),
    Boid(Vector2(1, -45), Vector2(2, 3),direction = World.CardinalDirection.East),
    Boid(Vector2(1, -45), Vector2(2, 3),direction = World.CardinalDirection.West)),
    noForces
  )

  generateTestCase(
    "64_4birdsWithDiffMigrationForcesAndRestOfTheForces",
    Seq(Boid(Vector2(1, -45), Vector2(2, 3),direction = World.CardinalDirection.North),
    Boid(Vector2(1, -45), Vector2(2, 3),direction = World.CardinalDirection.South),
    Boid(Vector2(1, -45), Vector2(2, 3),direction = World.CardinalDirection.East),
    Boid(Vector2(1, -45), Vector2(2, 3),direction = World.CardinalDirection.West)),
  ...
  generateTestCase(
    "65_AllBirdsAndBoids",
    Seq(Boid(Vector2(10, 40), Vector2(2, 3), direction = World.CardinalDirection.East),
    Boid(Vector2(-20, 25), Vector2(-2, -3), direction = World.CardinalDirection.East),
    Boid(Vector2(30, 0), Vector2(-2, 3), direction = World.CardinalDirection.East),
    Boid(Vector2(1, 15), Vector2(2, -3), direction = World.CardinalDirection.East),
    Boid(Vector2(1, -15), Vector2(2, 3)),
    Boid(Vector2(34, -45), Vector2(-2, 1)),
    Boid(Vector2(10, -145), Vector2(2, 1)),
    Boid(Vector2(-90, -95), Vector2(2, -1)),
    Boid(Vector2(120, 40), Vector2(-2, 3), direction = World.CardinalDirection.East),
    Boid(Vector2(-50, 55), Vector2(2, 3), direction = World.CardinalDirection.East),
    Boid(Vector2(30, -125), Vector2(-2, 3), direction = World.CardinalDirection.East),
    Boid(Vector2(100, -15), Vector2(2, 3), direction = World.CardinalDirection.East),
    Boid(Vector2(100, 15), Vector2(-2, 3)),
    Boid(Vector2(134, -45), Vector2(-2, -1)),
    Boid(Vector2(100, -145), Vector2(-1, 2)),
    Boid(Vector2(50, -95), Vector2(1, -1)),
    Boid(Vector2(365.3758239746094, 610.2220458984375), Vector2(-1.028688907623291, -0.12588320672512054)),
    Boid(Vector2(693.845947265625, 82.6295166015625), Vector2(-3.254211664199829, -3.422743558883667)),
    Boid(Vector2(119.18949890136719, 108.46981048583984), Vector2(-5.173732757568359, 2.5892333984375)),
    Boid(Vector2(924.3621826171875, 182.04296875), Vector2(2.597900390625, -3.3510782718658447)),
    Boid(Vector2(697.9754028320312, 133.5684356689453), Vector2(3.8886451721191406, 2.544830322265625)),
    Boid(Vector2(938.3258056640625, 178.064453125), Vector2(4.519933700561523, -4.376108169555664)),
    Boid(Vector2(190.23358154296875, 327.55364990234375), Vector2(-0.04482968896627426, -5.154569149017334)),
    Boid(Vector2(684.1359252929688, 605.9014892578125), Vector2(-0.03593646362423897, 1.7158024311065674)),
    Boid(Vector2(309.2644348144531, 157.3119659423828), Vector2(-5.749013423919678, 1.2016836404800415)),
    Boid(Vector2(611.5222778320312, 194.58639526367188), Vector2(1.334519863128662, -3.984072685241699)),
    Boid(Vector2(508.8362731933594, 419.8856506347656), Vector2(-1.2000545263290405, -2.2690560817718506), direction = World.CardinalDirection.North),
    Boid(Vector2(542.7695922851562, 82.1141128540039), Vector2(-0.9983347654342651, 0.5186262130737305), direction = World.CardinalDirection.North),
    Boid(Vector2(915.5891723632812, 539.8994750976562), Vector2(-2.7311182022094727, 1.1090834140777588), direction = World.CardinalDirection.North),
    Boid(Vector2(685.6277465820312, 674.078125), Vector2(4.7029194831848145, 4.29186487197876), direction = World.CardinalDirection.North),
    Boid(Vector2(745.296875, 97.20182800292969), Vector2(1.507777214050293, 0.5946908593177795), direction = World.CardinalDirection.North),
    Boid(Vector2(368.1731262207031, 563.56005859375), Vector2(4.484541416168213, 5.383422374725342), direction = World.CardinalDirection.North),
    Boid(Vector2(468.8144836425781, 661.2608032226562), Vector2(2.9881625175476074, -0.4230516850948334), direction = World.CardinalDirection.North),
    Boid(Vector2(82.65376281738281, 170.78587341308594), Vector2(-3.5700385570526123, 3.0642611980438232), direction = World.CardinalDirection.North),
    Boid(Vector2(839.0232543945312, 149.49737548828125), Vector2(-5.736677169799805, 0.6759297251701355), direction = World.CardinalDirection.North),
    Boid(Vector2(665.5927124023438, 478.4282531738281), Vector2(4.156680583953857, -0.8364709615707397), direction = World.CardinalDirection.North),
    Boid(Vector2(781.7577514648438, 380.0894775390625), Vector2(-1.879307508468628, 1.9312807321548462)),
    Boid(Vector2(414.48260498046875, 337.68017578125), Vector2(3.7263615131378174, 2.623392343521118)),
    Boid(Vector2(448.06390380859375, 284.6814270019531), Vector2(-2.9584689140319824, 3.849355459213257)),
    Boid(Vector2(218.42343139648438, 22.003005981445312), Vector2(-2.9874489307403564, 4.16240119934082)),
    Boid(Vector2(371.18994140625, 214.78878784179688), Vector2(-4.57440710067749, -0.19825108349323273), direction = World.CardinalDirection.East),
    Boid(Vector2(125.51665496826172, 401.0499267578125), Vector2(-3.4936325550079346, -6.162417888641357), direction = World.CardinalDirection.East),
    Boid(Vector2(208.02426147460938, 370.05792236328125), Vector2(-1.208107590675354, -1.4161628484725952), direction = World.CardinalDirection.East),
    Boid(Vector2(722.3165893554688, 629.8302001953125), Vector2(4.681003570556641, 0.17592816054821014), direction = World.CardinalDirection.East),
    Boid(Vector2(572.8057250976562, 662.74462890625), Vector2(-1.1091856956481934, -2.0724549293518066), direction = World.CardinalDirection.East),
    Boid(Vector2(151.72654724121094, 602.1031494140625), Vector2(-3.726334810256958, 3.288754463195801), direction = World.CardinalDirection.North),
    Boid(Vector2(877.9768676757812, 101.64559936523438), Vector2(4.357087135314941, -4.253688812255859), direction = World.CardinalDirection.North),
    Boid(Vector2(619.2003784179688, 36.4379997253418), Vector2(-4.238128662109375, 0.5205911993980408), direction = World.CardinalDirection.North),
    Boid(Vector2(627.57568359375, 620.370849609375), Vector2(1.6819838285446167, -2.4617979526519775)),
    Boid(Vector2(832.4192504882812, 70.95657348632812), Vector2(-1.3149347305297852, -1.5283052921295166))),
    allTogether
  )
