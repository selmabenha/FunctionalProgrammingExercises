...
final class FloatMatrixImage(
    private val data: Matrix[(Float, Float, Float, Float)]
) extends Image(data.width, data.height):

  def apply(x: Int, y: Int): Pixel =
    val (a, r, g, b) = data(x, y)
    argb(a.toInt, r.toInt, g.toInt, b.toInt)
