...
object ScalaShop:
  ...
  class ScalaShopFrame extends JFrame("ScalaShop\u2122"):
      ...
      new ActionListener():
        def actionPerformed(e: ActionEvent): Unit = getFilterName match
          ...
          case "edge-detection" =>
            ...
            setKernel(Kernel.sobelX, kernel1)
            setKernel(Kernel.sobelY, kernel2)
          case "median-filter" =>
            radiusLabel.setVisible(true)
            radiusSpinner.setVisible(true)
            kernel1Label.setVisible(false)
    ...
    stepbutton.addActionListener(
      new ActionListener:
        def actionPerformed(e: ActionEvent): Unit =
          ...
          updateInformationBox(s"$time ms")
    )
    ...
    def setKernel(kernel: Kernel, target: JTable) =
      require(kernel.width == target.getColumnCount())
      require(kernel.height == target.getRowCount())

      for r <- 0 until kernel.height do
        for c <- 0 until kernel.width do
          target.setValueAt(f"${kernel(c, r)}%.3f", r, c)
